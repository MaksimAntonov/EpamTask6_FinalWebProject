...
import static by.antonov.webproject.util.localization.LocalizationKey.TEXT_LOGIN_ERROR;
import static by.antonov.webproject.util.localization.LocalizationKey.TEXT_PROFILE_USER_BLOCKED_MESSAGE;
    ...
    try {
      ...
      if (userService.checkLogin(email, password) && (userOpt = userService.getUserByEmail(email)).isPresent()) {
        ...
        if (user.getUserStatus() != Status.BLOCKED) {
          session.setAttribute(USER_OBJ.name(), user);
          session.setAttribute(USER_ROLE.name(), user.getUserRole());
          ...
          router = new Router(RouterType.REDIRECT, RouterPath.CONTROLLER,
                              ...
                              KEY_PARAMETER_TRANSLATE_KEY.getValue() + "=" + TEXT_PROFILE_USER_BLOCKED_MESSAGE.name());
        }
      } else {
        router = new Router(RouterType.REDIRECT, RouterPath.CONTROLLER,
                            KEY_COMMAND.getValue() + "=go_to_login_page",
                            KEY_PARAMETER_STATUS.getValue() + "=error",
                            KEY_PARAMETER_TRANSLATE_KEY.getValue() + "=" + TEXT_LOGIN_ERROR.name());
      }
      return router;
